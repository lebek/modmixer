      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          {inactiveMods.map((mod) => (
            <ModRow
              key={`${mod.source}:${mod.folder}:${mod.about.packageIdLc}`}
              mod={mod}
              loadOrder={null}
        ...
        >
          {activeMods.map(({ packageId, mod, loadOrder }) => {
            if (!mod) {
              return (
                <MissingRow
                  key={`missing:${packageId}`}
                  packageId={packageId}
                  loadOrder={loadOrder}
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden border-r border-line last:border-r-0">
      ...
      <div className="flex-1 overflow-auto">
        {items === 0 ? (
          <div className="p-6 text-center text-xs text-muted">{empty}</div>
        ) : (
