...
Test-in-game flow when the user wants to run their mod:
1. Call run_test_cycle (folder = this mod's folder). It builds if needed, launches ./gradlew runClient with the diagnostics bridge, and arms background monitoring. The first run decompiles Minecraft and can take several minutes — that's expected, not a hang. By default the client auto-enters a fresh superflat creative world (cheats on) with the mod loaded; pass quicktest=false to stop at the title screen when the test needs the menus or a hand-made/existing world, or gameMode="survival" when it needs survival mechanics.
