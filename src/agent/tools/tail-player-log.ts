...
> = {
  ...
  description:
    "Read trailing lines from RimWorld's Player.log to find runtime errors. Provide a pattern (e.g. an exception class or a mod name) to focus on the relevant lines. When the pattern is a [Ref XXXXXXXX] tag, returns the ORIGINAL stack trace for that ref (RimWorld writes one full trace per ref and N 'Duplicate stacktrace' markers for every later occurrence; the tool resolves to the original automatically). Without a ref pattern, runs of duplicate-stacktrace markers are collapsed to a single ×N count. Use for ad-hoc diagnostics; for live monitoring during a test session use watch_player_log instead (it auto-prompts when errors arrive). Player.log is rewritten on each game launch, so call after the user has reproduced the issue.",
  parameters: Params,
